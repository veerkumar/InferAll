	...
	def  run(self, current_time):
		...
		if (self.last_read_element is None):
			if not task_queue.empty():
				...
				new_events.append((current_time + scheduler_wakeup_timer, PeriodicSchedulerEvent(self)))
				return new_events
		...
		if (requested_sleep_timer < 0):
			next_wakeup_time = scheduler_wakeup_timer
		else:
